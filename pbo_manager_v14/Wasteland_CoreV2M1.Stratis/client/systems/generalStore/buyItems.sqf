...
storePurchaseHandle = _this spawn
{
	...
	_showInsufficientFundsError =
	{
		...
		hint parseText format ["Não tem dinheiro suficiente para<br/>""%1""", _itemText];
	...
	{
		_itemText = _this select 0;
		hint parseText format ["Não tem espaço suficiente para<br/>""%1""", _itemText];
		playSound "FD_CP_Not_Clear_F";
		_price = -1;
	};
	...
	{
		_itemText = _this select 0;
		hint format ["""%1"" Foi Spawnado do lado de fora, em frente a loja", _itemText];
	...
	_showReplaceConfirmMessage =
	{
		...
		{
			_itemText = format ["Compando estes %1 Irá substituir os seus atuais.", _itemText];
		}
		else
		{
		...
		_confirmResult = [parseText _itemText, "Confirm", "Comprar", true] call BIS_fnc_guiMessage;
	...
	{
		_itemText = _this select 0;
		hint parseText format ["<t color='#ffff00'>Este item só pode ser pego<br/>completando uma missão.</t><br/>Procure missões no mapa<br/>para poder coleta-lo.", _itemText];
		playSound "FD_CP_Not_Clear_F";
		_price = -1;
	};
