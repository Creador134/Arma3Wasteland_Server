...
{
	["Você já está fazendo outra ação", 5] call mf_notify_client;
};
...
_checks =
{
	...
	switch (true) do
	{
		...
		case (isNull _object): { _text = "O item não existe mais" };
		case (alive _object || {alive _x} count crew _object > 0): { _text = "Ação Falhou! Você não pode salvar este item" };
		case (!isNull (_object getVariable ["R3F_LOG_est_deplace_par", objNull])): { _text = "Ação Falhou! Algém moveu o item" };
		case (!isNull (_object getVariable ["R3F_LOG_est_transporte_par", objNull])): { _text = "Ação Falhou! Alguém carregou ou rebocou o item" };
