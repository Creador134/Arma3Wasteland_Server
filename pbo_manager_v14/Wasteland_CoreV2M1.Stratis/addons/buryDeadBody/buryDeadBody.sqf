...
{
	_text = format ["O custo de enterrar um cadáver é $%1, você não tem dinheiro suficiente money!",_price];
	[_text, 5] call mf_notify_client;
};

_text = format ["\n\n\nEnterrar um cadáver irá lhe custar $%1. Você pode cancelar antes que chegue a 100%2 Completo!",_price,"%"];
...
private _checks =
{
	...
	switch (true) do
	{
		...
		default
		{
			_text = format ["Enterrando cadáver... %1%2 Completo",round(100 * _progress),"%"];
			_failed = false;
		};
	};
	[_failed, _text]
...
if (_outcome) then
{
	...
	{
		[" Alguém desenterrou o cadáver, você será reembolsado!", 5] call mf_notify_client;
	...
	{
		["Cadáver enterrado com sucesso", 5] call mf_notify_client;
	};
};
