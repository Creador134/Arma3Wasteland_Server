...
A3W_bleedingTime = 230;             // Time in seconds for which to allow revive after a critical injury (minimum 10 seconds)
...
A3W_atmMaxBalance = 16777216;       // Maximum amount of money that can be stored in a bank account (don't go over 16777216 as numbers start losing accuracy)
...
A3W_extDB_GhostingTimer = 0*60;    // Number of seconds a player has to wait when switching between servers running the same map (0 = disabled)
